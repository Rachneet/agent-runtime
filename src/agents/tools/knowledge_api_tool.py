...
logger = setup_logging(
    service_name="knowledge_search_tool", log_file="app.log", log_level="INFO"
)
...
class KnowledgeSearchInput(BaseModel):
    query: str = Field(
        description="Search query for finding relevant documentation, code, or data"
    )
...
class KnowledgeSearchResultItem(BaseModel):
    ...
    source_type: str = Field(
        description="The source category (e.g., 'code_repository', 'confluence')."
    )

    file_path: Optional[str] = Field(
        description="The relative file path or key (e.g., 'src/validator.py', 'QA/Standards')."
    )
    ...
    content_summary: str = Field(
        description="A short summary of the file/document's content for context."
    )
...
class KnowledgeAPISearchTool(BaseTool):
    ...
    description: str = (
        "Use this tool to search Riverty's unified knowledge base for documentation, code, and data."
    )
    ...
    def _run(
        ...
        results = self._client.search(query=query, sources=sources, limit=limit)
    ...
    def _extract_structured_data(self, api_results: dict) -> Dict[str, Any]:
        ...
        for res in raw_results:
            ...
            parsed_results.append(
                item.model_dump()
            )  # Use model_dump to get a clean dict
